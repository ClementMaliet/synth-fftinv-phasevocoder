...
parameter = Parameters(np.array([1]), np.array([0.12]), np.array([0.3]))
synth = StationarySynthesizer(1025, "hamming", 4, 500, 500, parameter)
synth.set_next_frame(parameter)
s = synth.get_next_frame()  # todo : Issues withe the phase vocoder
w = signal.get_window("hamming", 1025)
s_gt = np.array([1])*np.exp(2*np.pi*1j*0.12*np.array(range(1025)) + 1j)
s_gt *= w
plt.figure()
