...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.55]{grid.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{LUT.png}
	\caption{ Interpolation of stored lobe values onto the FFT bins}
\end{figure}
\newpage
...
The mains are located in the \texttt{example} sub-folder of the repository.
\section{Stationary Case}\label{sec:stationary-results}
\begin{figure}
	\centering
	\includegraphics[scale=0.3]{triangle.PNG}
	\caption{Two frame overlap-add reconstruction example}
	\label{statResult}
\end{figure}
\begin{figure}
	\centering
		\includegraphics[scale=0.4]{time.eps}
		\includegraphics[scale=0.4]{relativetime.eps}
		\caption{Time and Relative Time of Execution}
		\label{time}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[scale=0.4]{rmse.eps}
	\caption{RMSE of reconstruction function of the frequency}
	\label{precision}
\end{figure}
\sloppy
Stationary case has been tested for two consecutive frames in \texttt{main\_stationnary\_synthesis\_two\_frame.py} with their respective spectrum plotted, and with overlap-add in \texttt{main\_stationnary\_synthesis\_with\_overlapp.py}. 

The method works as shown in Figure \ref{statResult}. The execution time and precision analyses presented in Figure \ref{time} and \ref{precision} may be tested with \texttt{main\_compare\_with\_temporal\_synthesis.py}

In fact the true limitation of the method as of now in the stationary case in the computation time required for synthesis, which incompatible with real time by a comfortable margin. The precision however seems to very slightly outperform time-domain synthesis which is a promising result.

The \texttt{main\_stationnary\_synthesis\_with\_overlapp\_and\_analysis.py} is a test of the method on real quasi-stationary signals, using either the \texttt{estimation\_func.py} analysis method for non-stationary signals retrieved from the 2015 project or an elementary method located in \texttt{utils.py}\footnote{The method is non-functional as of now, for it lacks a sinusoid continuation algorithm to correctly propagates the phase}. Although the test seems not to work we left the file because we did not work on analysis, and everything seem to indicate that the error comes from the analysis part. Further work are is thus required to assess the validity of the method on real life sounds.
\section{Quasi-stationary and Non-Stationary Cases}\label{non-stationary-results}
The quasi-stationary synthesis has been tested with and without phase correction (Figure \ref{quasi-stationary}) for small frequency modulation ($\psi < 2000$) and surprisingly seems to perform significantly better without phase correction\footnote{Described in section \ref{quasi-stationary-case} page \pageref{quasi-stationary-case}}. This may be because of the inherent nature of phase coherence and a possible mistake in the phase correction term computed in section \ref{quasi-stationary-case}, if one wishes to check our results, details about phase advance computation may be found in appendix \ref{phase-advance} page \pageref{phase-advance}.

\sloppy
Both can be tested in the \texttt{main\_quasi\_stationary\_synthesis\_without\_phase correction.py} and \texttt{main\_quasi\_stationary\_synthesis\_with\_phase correction.py} files.

Non-stationary synthesis has been tested for two consecutive frames in \texttt{main\_non\_stationnary\_synthesis\_two\_frame.py} with their respective spectrum plotted, and with overlap-add in \texttt{main\_non\_stationnary\_synthesis\_with\_overlapp.py}. Although it does work, most probably because the LUT interpolation is either not precise enough or some mistakes are made at some points in the process. The code has been left as it is and may be ran to assess the errors, further work and corrections are required to make the method functional.
\begin{figure}
	\centering
	\includegraphics[scale=0.3]{quasistatwophasecorr.PNG}
	\includegraphics[scale=0.3]{quasistatwphasecorr.PNG}
	\caption{Two frame overlap-add reconstruction example for a quasi-stationary sinusoid}
	\label{quasi-stationary}
\end{figure}
\newpage
...
\begin{figure} [H]
	\centerline{
	\includegraphics[scale = 0.5] {packages_pyrevers_all.png}
}
\end{figure}
\newpage
\subsection{Class structure}\label{sec:class-structure}
Below is an Object Oriented public class diagram showing the relationships between the different classes. For more information on the inner working and the non-public elements of each classes, please refer to the code documentation part \ref{sec:documentation} page \pageref{sec:documentation}.
\begin{figure} [H]
	\centering
	\includegraphics[scale = 0.6] {code_structure.pdf}
\end{figure}
...
\texttt{%
	\begin{tabular}{|lc|}
		...
		\hdashline
		\_\_init\_\_(self, window\_size, window\_type, zero\_padding\_factor,\\ 
		analysis\_hop, synthesis\_hop, current\_parameters, fs=None) & \\
		\_\_del\_\_(cls) & \\
...
\texttt{your\_synthesized\_signal\_parameters = StationarySynthesizer(window\_size, window\_type, zero\_padding\_factor, analysis\_hop, synthesis\_hop, parameter)\\
your\_synthesized\_signal\_parameters.set\_next\_frame(parameter)
your\_synthesized\_signal = your\_synthesized\_signal\_parameters.get\_next\_frame()
}
...
\end{tabular}} \medskip\newline
To use it on a main.py file, just write for instance : \\
\texttt{your\_synthesized\_signal\_parameters = NonStationarySynthesizer(window\_size, window\_type, zero\_padding\_factor, analysis\_hop, synthesis\_hop, parameter)\\
your\_synthesized\_signal\_parameters.set\_next\_frame(parameter)
your\_synthesized\_signal = your\_synthesized\_signal\_parameters.get\_next\_frame()
}
...
Using add\_lobe method in a FOR loop, get\_lobe adds as many lobes as you want to a spectrum to create it.
...
\_gen\_lobe is an abstract class called either in StationaryLobeGenerator or in NonStationaryLobeGenerator that generates the lobe. Both methods will be more explained in the following paragraphs.
...
\_gen\_lobe first makes a circular shifting, that is to say that it turns the signal over so as to correct the phase. Then it computes the FFT on this new signal. Finally, it concatenates the 6 first points and the 5 last points of the signal to reconstruct the lobe.
...
The aim is to generate a LUT with a uniform or a non-uniform grid, and to generate a lobe for a given ACR/FCR couple by interpolating with existing lobes of the LUT. The user must choose the kind of interpolation he or she wants to use for the magnitude, the phase and the frequency. Besides, he can choose the size of the LUT to build by giving the number of ACRs and FCRs.\\
\_interpolate\_lobe defines the three interpolation methods to compute the amplitude, the phase or the frequency that the user can choose using scipy function interp2d.
...
To re-link this module on the whole structure, in the file \textbf{Synthesizer.py}, uncomment the lines 115- 119 - 120. \\
\texttt{
\# self.\_phase\_vocoder = StationaryPhaseVocoder(self.\_analysis\_hop, self.\_synthesis\_hop, self.\_current\_spectrum) \\
\# self.\_phase\_vocoder.current\_analysis\_spectrum = self.\_current\_spectrum \\
\# self.\_current\_spectrum = self.\_phase\_vocoder.get\_pv\_spectrum() \\
}
...
\hspace {15pt} Six weeks was a very short period for such a research subject. Indeed, we lost most of the time trying to understand the phase vocoder and its utility in the overall project. Moreover, we are not sure that it can work in real-time conditions because we could not evaluate the real performances of such a method. Indeed, we implemented it in Python which is a high-level interpreted programming language.
...
An interesting thing to do would be to get a real \textbf{analysis} method to get the parameters from a real sound signal and then to listen to the synthesized sound. Indeed, without an analysis tool, we cannot really test our synthesis method.
...
\subsection{Scale phase locking}
...
to channel $\Omega_{k1}$ is identified. The non-peak bins are said to
