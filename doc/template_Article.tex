...
\begin{equation*}
	s(t) = \sum_{i=1}^{N_{sinus}} \alpha_i \sin(2\pi f_i t+ \phi_i)
\end{equation*}
We then store the parameters as follows :
\begin{description}
	\item[\_amplitudes] stores the $\alpha_i$
	\item[\_frequencies] stores the $f_i$
...
\begin{equation*}
	s(t) = \sum_{i=1}^{N_sinus} (\alpha_i + \mu_i t) \sin(2\pi f_i t + \frac{\psi_i}{2} t^2 + \phi_i)
\end{equation*}
