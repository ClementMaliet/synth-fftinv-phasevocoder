...
When applying the STFT we may not, as opposed to section \ref{quasi-stationary-case}, assume the signal to be stationary in a small enough window, despite the slow variation of amplitude and frequency hypothesis. This is because the shape main lobe of the Fourier transform of the sinusoid is greatly dependant on amplitude and frequency modulation \cite{wellsmethods,wells2006high}, as showed on Figure .
...
An interesting thing to do would be to get a real \textbf{analysis} method to get the parameters from a real sound signal and then to listen to the synthesized sound. Because without an analysis tool, we cannot really test our synthesis method.
 ...
 In this project, we learned Python langage and how to manage the codes with GitHub. We also studied the phase of signals, something that in signal processing we didn't study much and which we don't really bother with.
