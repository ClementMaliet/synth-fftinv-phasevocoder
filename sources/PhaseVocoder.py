...
class PhaseVocoder(object):
    ...
    def __init__(self, analysis_hop, synthesis_hop, current_analysis_spectrum):
        if analysis_hop >= current_analysis_spectrum.get_nfft() or synthesis_hop >= current_analysis_spectrum.get_nfft():
            ...
            self._omega = np.array(range(current_analysis_spectrum.get_nfft()))*((2*np.pi)/current_analysis_spectrum.get_nfft())
